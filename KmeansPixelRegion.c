...
int main(int argc, char** argv)
{
  ...
  for(int i=0;i<K;i++){
    ...
    cMean[i].minLoc[0] = rnd_row;
    cMean[i].minLoc[1] = rnd_col;
    cMean[i].minDist = 99999;
  }
  ...
  while(!stopFlag){
    ...
    for(int i=0; i<img.height; i++){
      for(int j=0; j<img.width * 3; j+=3){
        ...
        if(minDistance<cMean[minC].minDist){
          cMean[minC].minDist = minDistance;
          cMean[minC].minLoc[0] = i;
          cMean[minC].minLoc[1] = j;
        }
      }
    }
    ...
    for(int i=0; i<img.height; i++){
      for(int j=0; j<img.width * 3; j+=3){
        ...
        for(int c=0;c<K;c++){
          int distance = sqrt(pow(img.pixels[i][j]-centers[c][0],2)+
            pow(img.pixels[i][j+1]-centers[c][1],2)+pow(img.pixels[i][j+2]-centers[c][2],2));
          loc = abs(i-cMean[c].minLoc[0]) + abs(j-cMean[c].minLoc[1]);
          if(distance*0.9+loc*0.1<minDistance){
            minDistance= distance;
            minC= c;
          }
        }
        regDistLocMap[i][j]= minC;
        regDistLocMap[i][j+1]= minDistance;
        regDistLocMap[i][j+2]= loc;
    ...
    if(diff>=1){stopFlag=0;printf("Continued...\n");}
    else{stopFlag=1;}
  }
